...
foreach ($columns as $column) {
    $checkColumn = $conn->query("SHOW COLUMNS FROM stella_payroll LIKE '$column'");
    if ($checkColumn->num_rows == 0) {
        $conn->query("ALTER TABLE stella_payroll ADD COLUMN $column VARCHAR(255) NOT NULL DEFAULT 'ONGOING'");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO stella_payroll (
                employee_id, 
                daily_rate, 
                rate_per_hour, 
                no_of_days, 
