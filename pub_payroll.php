...
<?php 
    ...
    p.others, p.cp, p.cash_advance, p.total_deductions, p.adjustment, p.net_pay
                ...
                AND e.branch = 'PUB'";  // Updated to filter by PUB branch
    ...
    p.others, p.cp,p.cash_advance, p.total_deductions, p.adjustment, p.net_pay
                ...
                FROM employees e
                LEFT JOIN pub_payroll p ON e.id = p.employee_id
                WHERE e.branch = 'PUB'";  // Updated to filter by PUB branch
...
/* Style the container holding both buttons */
.button-container {
...
/* Style for Download All Employees button */


/* Style for Clear All Data button */
#clearAllBtn {
    background-color: #28a745; /* Green */
    color: #fff;
    padding: 10px 20px;
...
/* Style for Download All Employees button */
#downloadAllBtn {
    background-color: #28a745; /* Green - matching Clear All Data button */
    color: white;
    padding: 10px 20px;
    font-size: 16px;
    font-weight: bold;
    border-radius: 25px; /* Matching the rounded corners of the Clear All Data button */
    border: none;
    cursor: pointer;
...
<body>
        ...
        <table>
            ...
            <tbody id="tableBody">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <td> 
                                <a href="view_pub_payroll.php?id=<?php echo $row['id']; ?>" class="view-btn">View</a>
                    ...
                    echo "<tr><td colspan='23'>No employees found in PUB branch</td></tr>"; 
                }
                ?>
            </tbody>
        </table>
    ...
    window.location.href = "view_pub_payroll.php";
}
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', 'save_payroll_changes_pub.php', true);
    xhr.onload = function () {
        if (xhr.status === 200) {
